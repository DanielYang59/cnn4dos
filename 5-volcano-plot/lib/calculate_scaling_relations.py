...
class scalingRelations:
    def __init__(self, free_energy_linear_relation, adsorbate_energy_file, reaction_pathway_file, external_potential, verbose=True, debug=False):
        """Calculate free energy scaling relations of each reaction step.
        ...
        Args:
            ...
            debug (bool, optional): debug mode, intermediate results would be saved. Defaults to False.
        ...
        self.adsorbate_energy_dict = self.import_adsorbate_free_energy(adsorbate_energy_file)
    ...
    def calculate_half_equation(self, equation_part):
        """Calculate free energy term for half of reaction equation.

        Args:
            equation_part (dict): dict of half reaction equation
        ...
        for species, num in equation_part.items():            
            ...
            # pristine catalysts surface (ignored, as will cancel out)
            elif species == "*":
                pass
            ...
            # non-adsorbed species (free molecule or non-molecular adsorbate)
                ...
                species = species.split("_")[0]  # remove _is suffix
    ...
    def calculate_relations(self, reaction_name):
        """Calculate scaling relations for selected reaction.
        ...
        """
        # Check args
        if reaction_name not in self.reaction_pathway_dict:
            print(f"Cannot find reaction pathway data for reaction {reaction_name}.")
        

        # Calculate linear relation for each reaction step
        if self.verbose:
            print(f"Calculating free energy linear relations for reaction \"{reaction_name}\".")
    ...
    def import_adsorbate_free_energy(self, file):
        ...
        # Import adsorbate free energy csv file and pack into dict
        df = pd.read_csv(file)
