...
class occlusionGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            occlusion_width (int): The width of the occlusion window.
                Actual width is `occlusion_width * dos_calculation_resolution`.
            occlusion_step (int): The step size for moving the occlusion
                window. Actual step size is
                `occlusion_step * dos_calculation_resolution`.
            dos_calculation_resolution (float):
                The resolution for eDOS calculations.

        Raises:
            ValueError: When input values are not as expected.
            warnings.Warn: When the number of samplings
    ...
    def _occlude(
        ...
        Returns:
            np.ndarray: The occluded eDOS array, maintaining the
                same shape as the original eDOS array.
        """
    ...
    def generate_occlusion_arrays(self) -> np.ndarray:
        """
        Generate a series of occlusion eDOS arrays, where each array is
        created by occluding a region in the original eDOS array
        (not going across orbitals).

        Returns:
            np.ndarray: An array of shape (num_occlusions, numOrbitals),
                each element hosting an occluded_array of
                shape (numSamplings, numOrbitals)
        """
