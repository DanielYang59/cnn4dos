...
def hp_model(hp, input_shape=(4000, 9, 6)):
    def branch(branch_input, drop_out_rate, numFilters=16):
        ...
        for _i in range(hp_branch_numConvBlocks):
            for _j in range(hp_branch_primary_numConvLayers):
                # Primary ConV blocks
                conv_x = tf.keras.layers.Conv2D(
                    numFilters,
                    (hp_branch_primary_kernel_size, 1),
                    activation="relu",
                    padding="same",
                )(conv_x)
            ...
            conv_x = tf.keras.layers.Conv2D(
                numFilters,
                (hp_branch_ending_kernel_size, 1),
                (2, 1),
                activation="relu",
                padding="same",
            )(conv_x)
            ...
            if hp_branch_include_pooling:
                conv_x = tf.keras.layers.AveragePooling2D(
                    pool_size=(hp_branch_pool_size, 1), padding="same"
                )(conv_x)
    ...
    # )
    # hp_drop_out_rate = hp.Float(
    #     "drop_out_rate", min_value=0.1, max_value=0.5
    # )
    ...
    # )
    hp_master_activation_function = hp.Fixed(
        "hp_master_activation_function", value="relu"
    )

    # Branch
    # hp_branch_dense_activation_func = hp.Choice(
    #     "hp_branch_dense_activation_func", ["tanh", "relu", "sigmoid"]
    # )
    hp_branch_dense_activation_func = hp.Fixed(
        "hp_branch_dense_activation_func", value="tanh"
    )
    hp_numFilters = hp.Int("hp_numFilters", min_value=8, max_value=64)
    # hp_numFilters = hp.Fixed("hp_numFilters", value=12)
    hp_branch_primary_kernel_size = hp.Int(
        "hp_branch_primary_kernel_size", min_value=2, max_value=32, step=2
    )
    # hp_branch_primary_kernel_size = hp.Fixed(
    #     "hp_branch_primary_kernel_size", value=22
    # )
    hp_branch_ending_kernel_size = hp.Int(
        "hp_branch_ending_kernel_size", min_value=2, max_value=32, step=2
    )
    hp_branch_primary_numConvLayers = hp.Int(
        "hp_branch_primary_numConvLayers", min_value=1, max_value=16
    )

    hp_branch_numConvBlocks = hp.Int(
        "hp_branch_numConvBlocks", min_value=1, max_value=16
    )

    hp_branch_include_pooling = True

    if hp_branch_include_pooling:
        hp_branch_pool_size = hp.Int(
            "hp_branch_pool_size", min_value=2, max_value=32, step=2
        )

    hp_branch_dense_units = hp.Choice(
        "hp_branch_dense_units", [16, 32, 64, 128, 256, 512]
    )
    ...
    branch_output_0 = branch(
        (master_input[:, :, 0]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_1 = branch(
        (master_input[:, :, 1]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_2 = branch(
        (master_input[:, :, 2]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_3 = branch(
        (master_input[:, :, 3]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_4 = branch(
        (master_input[:, :, 4]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_5 = branch(
        (master_input[:, :, 5]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_6 = branch(
        (master_input[:, :, 6]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_7 = branch(
        (master_input[:, :, 7]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    branch_output_8 = branch(
        (master_input[:, :, 8]),
        drop_out_rate=hp_drop_out_rate,
        numFilters=hp_numFilters,
    )
    ...
    master_output = tf.keras.layers.Dense(
        hp_master_1st_dense_units, activation=hp_master_activation_function
    )(master_flat)
    master_output = tf.keras.layers.Dense(
        hp_master_2nd_dense_units, activation=hp_master_activation_function
    )(master_output)
