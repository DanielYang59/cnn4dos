...
def cnn_for_dos(
    input_shape: tuple[int], drop_out_rate: float
) -> tf.keras.Model:
    """Definition of the multi-branch CNN model.
    ...
    def branch(branch_input, drop_out_rate: float):
        ...
        branch_input = tf.keras.layers.Reshape(
            target_shape=(4000, 1, 6)
        )(branch_input)

        # 1st Conv layer
        conv_1 = tf.keras.layers.Conv2D(
            16, (3, 1), activation="relu", padding="same"
        )(branch_input)
        conv_1 = tf.keras.layers.Conv2D(
            16, (3, 1), activation="relu", padding="same"
        )(conv_1)
    ...
    branch_output_0 = branch(
        (master_input[:, :, 0]), drop_out_rate=drop_out_rate
    )
    branch_output_1 = branch(
        (master_input[:, :, 1]), drop_out_rate=drop_out_rate
    )
    branch_output_2 = branch(
        (master_input[:, :, 2]), drop_out_rate=drop_out_rate
    )
    branch_output_3 = branch(
        (master_input[:, :, 3]), drop_out_rate=drop_out_rate
    )
    branch_output_4 = branch(
        (master_input[:, :, 4]), drop_out_rate=drop_out_rate
    )
    branch_output_5 = branch(
        (master_input[:, :, 5]), drop_out_rate=drop_out_rate
    )
    branch_output_6 = branch(
        (master_input[:, :, 6]), drop_out_rate=drop_out_rate
    )
    branch_output_7 = branch(
        (master_input[:, :, 7]), drop_out_rate=drop_out_rate
    )
    branch_output_8 = branch(
        (master_input[:, :, 8]), drop_out_rate=drop_out_rate
    )
