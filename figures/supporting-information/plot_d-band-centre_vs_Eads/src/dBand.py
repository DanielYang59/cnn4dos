...
class dBand:
    ...
    def __calculate_band_centre(self, single_dos_orbital, energy_array):
        ...
        numerator = np.trapz(
            y=(np.copy(single_dos_orbital) * np.copy(energy_array)),
            dx=(self.energy_range[1] - self.energy_range[0]) / self.nedos,
        )

        denominator = np.trapz(
            y=single_dos_orbital,
            dx=(self.energy_range[1] - self.energy_range[0]) / self.nedos,
        )
    ...
    def __take_d_band(self):
        ...
        if self.numOrbitals in {1, 4}:
            d_band = np.zeros(
                (self.nedos, 5)
            )  # generate zero array when no d-orbital presents
    ...
    def calculate_d_band_centre(self, merge_suborbitals=True, verbose=False):
        ...
        d_band_centre = self.__calculate_band_moment(
            merged_d_band, energy_array, ordinal=1
        )
    ...
    def calculate_d_band_width(self, merge_suborbitals=True, verbose=False):
        ...
        d_band_width = self.__calculate_band_moment(
            merged_d_band, energy_array, ordinal=2
        )
