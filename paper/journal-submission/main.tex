...
\begin{abstract}
    ...
    This work offers a tangible framework to guide both theoretical screening and experimental synthesis,
...
\section{Introduction}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=\textwidth]{main_fig1_pipeline.pdf}
        \caption{\textbf{Catalyst performance analysis pipeline integrating volcano plots and CNN.}
        The figure demonstrates the integrated usage of volcano plots for predictive assessment of existing catalysts, and the utility of CNNs to predict and modulate adsorption energies \(E_{\text{ads}}\) using eDOS as input.
    ...
    Computational modeling, particularly methods based on quantum mechanics (QM) such as density functional theory (DFT), allows for the profiling the energetics of intermediates and unveils structure-activity relationships for identified active sites \cite{feaster2017understanding, carter2008challenges}, contributing to understanding of reaction mechanisms.
    ...
    To streamline this featurization process, we employed CNNs, which autonomously derive a hierarchical representation directly from the eDOS \cite{tran2015learning, socher2012convolutional, NIPS2012_c399862d}.
    ...
    Evaluations on the adsorption energies of nine intermediates along CO\textsubscript{2}RR and competing hydrogen evolution reaction (HER) as well yielded prediction mean absolute errors (MAEs) at the level of 0.1 eV, which is well aligned with the precision of conventional DFT calculations \cite{kirklin2015open, lejaeghere2016reproducibility, wellendorff2015benchmark}.
    This result validated the capability of current CNN model to accurately capture intricate spatial information from eDOS.
    Building on this, we introduced an orbital-wise eDOS occlusion experiment, inspired by the occlusion technique commonly used in computer vision and pattern recognition.
    ...
    In summary, we introduced a catalyst performance analysis pipeline by integrating CNN and volcano plot analysis, using CO\textsubscript{2}RR as an example proof of concept.
...
\subsection{Adsorption energy prediction from electronic density of states with convolutional neural network}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=\textwidth]{main_fig2_cnn.pdf}
        \caption{\textbf{CNN for electronic density of states.}
        \textbf{a}. Kendall rank correlation coefficient map illustrating the correlation between adsorption energy and other electronic and elementary descriptors.
        Definitions of the notations are provided in Supplementary \cref{supp_table15:descriptor_notations}.
        \textbf{b}-\textbf{c}. Architectural of (\textbf{b}) the root and (\textbf{c}) each branch of the CNN model.
        The architecture stays consistent across individual branches.
        \textbf{d}-\textbf{m}. Parity plots demonstrating the performance of the CNN model across nine adsorbates:
    ...
    However, d-band center alone does not suffice for accurate predictions of intermediate adsorption energy as illustrated in the scatter plot Supplementary \cref{supp_fig24:dband_vs_eads}, and therefore, refinements are imperative.
    ...
    Specifically, CNNs can automatically identify spatial patterns and complex features from raw data, which reduces the need for manual feature engineering.
    This ability to model nonlinear relationships, critical in electrochemical processes, sets CNNs apart from traditional machine learning models like support vector machines or random forests.
    These strengths are especially important in optimizing CO2 reduction reactions, where atomic arrangements, electron densities, and surface morphology play significant roles.
    ...
    This makes our model flexible and adaptable to other material systems beyond 2D materials, with the proper fine-tuning of hyperparameters.
    Given the high-dimensional nature of eDOS, we believe our model holds promise for broader applications, though this would need to be validated with additional datasets representing other material classes.
...
\subsection{Hybrid descriptors enabled volcano plot for catalyst analysis}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{main_fig3_volcano.pdf}
    ...
    Catalyst activity volcano plots, built upon the scaling relations proposed by Peterson and Nørskov's \cite{peterson2012activity}, goes beyond assessment of candidatures' energetics only but to provide informative guidance on discovering catalysts candidates with superior performance \cite{balandin1969modern, deutschmann2000heterogeneous}.
    ...
    CO is a critical intermediate in CO\textsubscript{2} reduction, especially for reactions leading to multi-carbon products, while OH intermediates are involved in competing pathways that influence selectivity and reaction kinetics.
    These descriptors allow us to gain mechanistic insights into both the reaction environment and the catalysts' electronic interactions with intermediates.
    By integrating both bonding featured descriptors into the hybrid approach, we found the $R^2$ was increased by 0.1387.
    Detailed information about the scaling parameters and performance enhancements are provided in Supplementary \cref{supp_table13:scaling_params} and Supplementary \cref{supp_fig16:r2_hyb_des}.
    ...
    Expanding upon the revised scaling relations, we introduce an activity volcano plot and a rate-determining step (RDS) volcano plot for the CO\textsubscript{2}RR process, shown in \cref{main_fig3:volcano}a and Supplementary \cref{supp_fig17:rds_volcano}.
    Given that HER is a key competing reaction with CO\textsubscript{2}RR \cite{goyal2020competition}, we also incorporated a selectivity volcano plot, presented in Supplementary \cref{supp_fig18:sel_volc}, to offer a comprehensive analysis of catalyst candidates.
    ...
    Results from RDS volcano plot indicate that protonation of *CO likely serves as the RDS for CO\textsubscript{2}RR. Consequently, it should be the focal point for future catalyst designs.
    ...
    This result suggests a perspective: weak binding of CO to the catalyst surface might facilitate the rotation of C-O backbone from the ``upright'' configuration in *CO to the ``horizontal'' orientation in *CHO \cite{peterson2010copper}, and therefore facilitating dynamics and protonation of the *CO species.
    ...
    While the volcano plot offers an intuitive analysis of the reaction energetics and provides insights into the mechanisms of the reaction, it lacks the ability to delve into lower-level explanations rooted in electronic structures, where detailed electronic interactions and specific bonding mechanisms remain beyond its scope.
    ...
    Additionally, we mapped the theoretical limiting potentials of the analyzed catalyst candidates onto periodic tables, shown in \cref{main_fig3:volcano}b and Supplementary \cref{supp_fig19:n-gra_ptable} to Supplementary \cref{supp_fig23:C2N_ptable}.
...
\subsection{Validation of CNN predictions}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=\textwidth]{main_fig4_validation.pdf}
        ...
        (\textbf{d}) Cr $d_{yz}$ orbitals of Cr@g-C\textsubscript{3}N\textsubscript{4},
    ...
    A visual representation of this process can be found in Supplementary \cref{supp_fig27:occlusion}. Conceptually, this experiment simulates the effect of shielding electronic states, allowing investigation of the potential contribution to adsorption energy from these states.
    ...
    The $d_{yz}$ and $d_{z^2}$ orbitals emerge as dominant in CO interaction.
    Shielding electron states centered around -2 eV significantly alter the adsorption energy, indicating the potency of these orbitals.
    The real-space wavefunction visualization on $d_{yz}$ orbital in \cref{main_fig4:validation}d corroborates these findings, highlighting the capability of occlusion experiment to identify contribution from specific orbitals with certain spatial alignment.
    Additionally, occlusion experiments were performed on the Co atom of Co@g-C\textsubscript{3}N\textsubscript{4} (\cref{main_fig4:validation}b) and the Cu atom of Cu@g-C\textsubscript{3}N\textsubscript{4} (\cref{main_fig4:validation}c), in both these, the $d_{xy}$ orbitals were identified to dominate the catalysts-adsorbate interaction.
    ...
    The results of the occlusion experiment on the Co $d_{xy}$ orbital of Co@g-C\textsubscript{3}N\textsubscript{4}, identified as the dominant orbital via occlusion experiments, were compared with the COHP analysis, as shown in \cref{main_fig4:validation}g and \cref{main_fig4:validation}i.
    ...
    We then investigated the non-dominant Co $d_{xz}$ orbital, and the COHP analysis highlighted interactions at -1 eV and -6.5 eV, aligning with the findings from the occlusion experiments.
...
\subsection{eDOS shifting experiments for prediction of better catalysts}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=\textwidth]{main_fig5_shifting.pdf}
        ...
        The disturbances caused by shifting of (\textbf{a}) entire d, (\textbf{b}) $d_{xy}$, (\textbf{c}) $d_{yz}$, (\textbf{d}) $d_{z^2}$, (\textbf{e}) $d_{xz}$ and (\textbf{f}) $d_{x^2-y^2}$ orbitals are illustrated.
        The shifting step size corresponds to the energy resolution of the eDOS, and is 0.005 eV in this study.
    ...
    We also investigated potential disturbance from p orbital shifting, as shown in Supplementary \cref{supp_fig37:p_shifting} and found no significant response.
    ...
    In summary, shifting experiments offer a valuable means to predict the potential disturbances in adsorption behavior resulting from eDOS shifting, a task that is challenging to accomplish directly within the DFT framework.
    ...
    Skillful manipulation of the electronic structure of catalyst candidates would be required, presenting a practical challenge.
    At present direct control over the electronic density of states in actual catalytic materials remains an open area of research.
    But emerging approaches, such as defect engineering, doping, and strain modulation, offer potential pathways for indirectly influencing eDOS in real materials.
    These methods have been used to adjust the electronic structure and, consequently, the catalytic activity of materials. While these methods do not enable direct eDOS shifting, but the development of advanced techniques such as in situ spectroscopic methods (e.g., X-ray absorption spectroscopy and electron energy loss spectroscopy) could provide valuable insights into how the eDOS can be tuned, particularly in the context of SACs, where the localized electronic environment around single metal atoms can be finely controlled.
    In contrast to prior studies on bulk metals \cite{fung2021machine}, wherein the adsorption energy exhibits a continuous shift with eDOS variations, the response of SACs is notably discrete.
    ...
    The CNN model used in our study operates in a forward direction—mapping eDOS to predicted catalytic activity (i.e., binding energies).
    However, this approach does not provide a direct pathway to reverse-engineer material structures that possess the desired eDOS characteristics.
    Although a material recommendation model is not explicitly developed here, we acknowledge that generative models such as Variational Autoencoders (VAEs) and Generative Adversarial Networks (GANs) have shown promising results in the domain of materials discovery.
    These models can be trained to generate candidate structures that optimize certain target properties, including electronic characteristics like eDOS.
    For instance, GANs have been successfully employed to propose novel materials for various applications by learning from existing data and extrapolating structures that satisfy specific criteria.
    In the case of eDOS-guided catalyst design, a reverse design model could be built by training generative networks on materials datasets with known eDOS properties.
    Such a model could then suggest new materials that achieve desirable eDOS profiles for catalytic activity in CO\textsubscript{2} reduction, oxygen evolution reaction (OER), and other reactions.
...
\section{Methods}
    ...
    Supported SACs were modelled by anchoring individual metal atoms onto various 2D substrates, including g-C\textsubscript{3}N\textsubscript{4}, nitrogen-doped graphene, graphene with dual-vacancy, black phosphorous, boron nitride, and monolayer C\textsubscript{2}N.
    ...
    The C1 production CO\textsubscript{2}RR pathway, as initially described by Peterson et al. \cite{peterson2010copper} and later confirmed by Jiao et al. \cite{jiao2017molecular} for
    ...
    Hybrid descriptors incorporate both descriptors within a single equation, enhancing the accuracy of scaling relationship with minimal computational overhead, as demonstrated in Supplementary \cref{supp_fig16:r2_hyb_des} and Supplementary \cref{supp_table13:scaling_params}.
    ...
    And we also defined the CO\textsubscript{2}RR vs HER selectivity as:
    \begin{align}
    U_{sel} = U_{L_{CO_{2}RR}} - U_{L_{HER}}  \label{main_eq16:selectivity}
    \end{align}

    This approach simplifies the theoretical performance assessment of any catalyst within the scope of our research to only two descriptors:
    ...
    To enhance the generalizability and robustness of the our CNN model, and achieve comprehensive coverage of the chemical space \cite{DBLP:journals/corr/abs-2112-12542}, we applied data augmentation techniques to the initial dataset using the following procedures:
        - In addition to the designated ``initial state'' of the adsorption process, where the adsorbate positioned 6.5 \text{\AA} above the supported single metal atom, five intermediate images were interpolated.
        These images were distributed between the initial and final states at a spacing of 0.5 \text{\AA}, starting from the initial state side.
        - A diverse set of smearing techniques was employed to determine of partial occupancies $f_{nk}$, including the tetrahedron method \cite{blochl1994improved} and the Gaussian method.
        For the Gaussian method, the smearing width was varied within the range of 0.01 eV to 0.1 eV.
        This careful selection of methods was made to guarantee reliable and consistent predictions of adsorption energies from the model, regardless of the smearing technique used.
    ...
    Supplementary \cref{supp_fig27:occlusion} visually illustrates the eDOS occlusion experiment methodology.
    ...
    Supplementary \cref{supp_fig36:shifting} illustrates the implemented eDOS shifting experiment protocol.
...
\newpage
    ...
    Essential source code and data required to replicate the study's results-including VASP input files, structure files of SACs, eDOS arrays and the pretrained CNN model-are available on GitHub repository at \url{https://github.com/DanielYang59/cnn4dos}, and can be provided by the corresponding authors upon request.
    Due to their substantial size in terabytes, the complete source data (complete VASP output files and such) is not currently hosted on publicly available repositories, but can be obtained from the corresponding authors upon request.


    \section{Acknowledgements}
    We acknowledge funding support from the Australian Research Council (FT160100281, FT180100387, DP200103568, DP230101625).
    ...
    Dr. Ryky Nelson from RWTH Aachen University offered valuable guidance on Crystal Orbital Hamilton Population (COHP) analysis.


    \section{Author Contributions}
    H.Y. performed DFT calculations, implemented Python code for machine learning methodologies and data analysis, and drafted and refined the manuscript.
